...
class TaggingsController extends AppController
{
	...
	public function saveajax(){
			$this->ajaxLayout();
			 ...
			 $tagging = $this->Taggings->newEmptyEntity();
			 if ($this->request->is('ajax')) {
				//$this->log(json_encode($this->request->getData()));
				$tagging = $this->Taggings->patchEntity($tagging, $this->request->getData());
				$resp 	= 0;
				
				if ($this->Taggings->save($tagging)) {
					$resp 	= 1;
					...
					//$this->log(json_encode($this->request->getData()));
					$err = '';
					if($tagging->getErrors()){
						//$this->log(json_encode($tagging->getErrors()));
						$error_msg = [];
						foreach( $tagging->getErrors() as $errors){
							if(is_array($errors)){
   ...
   public function indexajax(){
		...
		if($this->request->is('ajax')){
			...
			$q1 					= "SELECT count(*) as allcount FROM taggings";
			$records 				= $db->execute($q1)->fetchAll('assoc');
			$totalRecords 			= $records[0]['allcount'];
			
			$q2 					= "SELECT count(*) as allcount FROM taggings as c " . $searchQuery;
			...
			$empQuery = "SELECT c.* from taggings as c ".$searchQuery." 
			...
			if(!empty($empRecords)):
				foreach($empRecords as $c):
				  $data[] = array( 
						...
						'name'			=> $c['name'],
						'stat'			=> $c['status'],
						'action' 		=> '<a href="../taggings/edit/'.$c['id'].'" data-table = "tagging_table" 
			                                data-controller = "taggings" title="Update Tagging Information" note="Required fields are marked with *" data-toggle="modal" data-target="#form_content_sub"  class="modal_view_sub"><i class="fa fa-eye fa-lg"></i></a>'	
				   );
				endforeach;
			endif;	
	...
	public function index(){
        ...
        $tagging = $this->paginate($this->Taggings);
