...
class SubitemsController extends AppController
{
	...
	public function saveajax(){
			$this->ajaxLayout();
			 ...
			 $subitem = $this->Subitems->newEmptyEntity();
			 if ($this->request->is('ajax')) {
				//$this->log(json_encode($this->request->getData()));
				$subitem = $this->Subitems->patchEntity($subitem, $this->request->getData());
				$resp 	= 0;
				
				if ($this->Subitems->save($subitem)) {
					$resp 	= 1;
					...
					//$this->log(json_encode($this->request->getData()));
					$err = '';
					if($subitem->getErrors()){
						//$this->log(json_encode($subitem->getErrors()));
						$error_msg = [];
						foreach( $subitem->getErrors() as $errors){
							if(is_array($errors)){
   ...
   public function indexajax(){
		...
		if($this->request->is('ajax')){
			...
			$q1 					= "SELECT count(*) as allcount FROM subitems";
			$records 				= $db->execute($q1)->fetchAll('assoc');
			$totalRecords 			= $records[0]['allcount'];
			
			$q2 					= "SELECT count(*) as allcount FROM subitems as c " . $searchQuery;
			...
			$empQuery = "SELECT c.* from subitems as c ".$searchQuery." 
			...
			if(!empty($empRecords)):
				foreach($empRecords as $c):
				  $data[] = array( 
						...
						'name'			=> $c['name'],
						'stat'			=> $c['status'],
						'action' 		=> '<a href="../subitems/edit/'.$c['id'].'" data-table = "subitem_table" 
			                                data-controller = "subitems" title="Update Sub-item Information" note="Required fields are marked with *" data-toggle="modal" data-target="#form_content_sub"  class="modal_view_sub"><i class="fa fa-eye fa-lg"></i></a>'	
				   );
				endforeach;
			endif;	
	...
	public function index(){
        ...
        $subitem = $this->paginate($this->Subitems);
