	...
	/*$(document).on("hidden.bs.modal", "#form_content_with_place", function(){
		$("#p_table").DataTable().ajax.url(getTheWebroot()  + "couriertxtitems/indexajax").load();
	});*/
	...
	$(document).on("click", ".modal_view", function(e){
		...
		$.get(url, function(resp){
			$(".modal-body").html(resp).promise().done( function(){
				...
				//uploadImage(_webroot + "couriertxtitems/postimage/", "png, jpg", "#product_image");
				
				$("#couriercontract_id").change( function(){
					var contract = $("#couriercontract_id option:selected").val();
					if(contract !==""){
						
						massUploadForm(_webroot + "couriertxtitems/importproducts/", "csv", contract);
					}
				});
		...
		$.get(url, function(resp){
			...
			$(".modal-body-sub").html(resp).promise().done( function(){
				...
				$("#btnfilter").click( function(e){
					...
					var url = "couriertxtitems/indexajax/filter/" + program_id + "/" + category_id + "/" + vendor_id + "/" + tagging_id + "/" + subcategory_id
