	...
	*/
	...
	$(document).on("click", ".refresh_table", function(e){
		...
		var url = getTheWebroot()  + "logistics/indexajax/";
		refReshTable(url);
		...
		$.get(url, function(resp){
			$(".modal-body").html(resp).promise().done( function(){
				...
				$("#btnreceiveall").click( function(){
					...
					var _conf = confirm("You are about to receive the selected item. Please click OK to confirm");	
					if(_conf){
						showLoading("show");
						$.post(url, postdata, function(resp){
							var res = JSON.parse(resp);
		...
		$.get(url, function(resp){
			...
			$(".modal-body-sub").html(resp).promise().done( function(){
				...
				$("#btnfilter").click( function(e){
					...
					var url = "logistics/indexajax/filter/" + filter_type + "/" + program_id + "/" + category_id + "/" + vendor_id + "/" + tagging_id + "/" + subcategory_id + "/" + school + "/" + warehouse + "/" + status + "/" + region + "/" + province +"/"+ city +"/"+ barangay +"/"+ d_from +"/"+ d_to;
	...
	$(document).on("click", ".view_details_vstat", function(e){
		...
		var url = "logistics/indexajax/filter/status/0/0/0/0/0/0/0/" + status + "/0/0/0/0/0/0";
